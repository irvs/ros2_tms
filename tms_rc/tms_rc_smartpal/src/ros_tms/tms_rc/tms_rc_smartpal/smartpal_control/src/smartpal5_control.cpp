...
// public:
//   SmartpalControl()
//   : Node("smartpal_control_node"), count_(0)
//   {
//     // initialize();
...
//     // auto get_data_client = this->create_client<tms_msg_db::srv::TmsdbGetData>("/tms_db_reader");



//     // auto arm_data_sub = this->create_subscription<sensor_msgs::msg::JointState>("/move_group/fake_controller_joint_states", 10, &SmartpalControl::armcallback);
//     // auto object_data_sub = this->create_subscription<moveit_msgs::msg::PlanningScene>("/move_group/monitored_planning_scene", 10, std::bind(&SmartpalControl::ObjectDataUpdate, this, _1));
...
//     while (!get_data_client->wait_for_service(std::chrono::seconds(10))) {
//     printf("2.\n");
//     //                       printf("CORBA client object initialization has been failed.\n");
...
//       // listener->tf2::doTransform("/world_link", pose, pose2);
//       auto node = rclcpp::Node::make_shared("minimal_client0");
//       auto get_data_client1 = node->create_client<getRobotData>("add_two_ints");
//       cout  << "client 1" << endl;
...
//       while (!get_data_client1->wait_for_service(std::chrono::seconds(1))) {
//         if (!rclcpp::ok()) {
//           RCLCPP_ERROR(this->get_logger(), "client interrupted while waiting for service to appear.");
//           return;
//         }
...
//       auto result_future = get_data_client1->async_send_request(srv);
...
//       auto get_data_client2 = node->create_client<getRobotData>("add_two_ints");
//       cout  << "client 2" << endl;
      
//       while (!get_data_client2->wait_for_service(std::chrono::seconds(1))) {
//         if (!rclcpp::ok()) {
//           RCLCPP_ERROR(this->get_logger(), "client interrupted while waiting for service to appear.");
//           return;
//         }
...
//       tms_msg_db::srv::TmsdbGetData::Response res;
//       // srv.tmsdb.id = object_id + sid_;
//       auto result_future = get_data_client2->async_send_request(srv);
//       auto status = result_future.wait_for(std::chrono::seconds(3));
//       if(status == std::future_status::ready)
//       {
//         // g_oz -= srv.response.tmsdb[0].offset_z;
...
//   rclcpp::Publisher<tms_msg_db::msg::TmsdbStamped>::SharedPtr pose_publisher;
//   // rclcpp::Client<tms_msg_db::srv::TmsdbGetData>::SharedPtr get_data_client;
//   rclcpp::Subscription<sensor_msgs::msg::JointState>::SharedPtr arm_data_sub;
//   rclcpp::Subscription<moveit_msgs::msg::PlanningScene>::SharedPtr object_data_sub;
...
// class SmartpalSubscriber : public rclcpp::Node
// {
// public:
//   SmartpalSubscriber()
//   : Node("subscriber1")
...
//     // auto object_data_sub = this->create_subscription<moveit_msgs::msg::PlanningScene>("/move_group/monitored_planning_scene", 10, std::bind(&SmartpalControl::ObjectDataUpdate, this, _1));
//     auto arm_data_sub = this->create_subscription<sensor_msgs::msg::JointState>("/move_group/fake_controller_joint_states", 10, std::bind(&SmartpalSubscriber::armcallback, this, _1));
...
int main(int argc, char * argv[])
{
  ...
  // auto subscription = std::make_shared<SmartpalSubscriber>();

  // auto subscription1 = rclcpp::Node::make_shared("arm_data_node");
  // auto subscription2 = rclcpp::Node::make_shared("object_data_node");

  // auto timer_ = publisher->create_wall_timer(500ms, timer_callback);
  // rclcpp::WallRate loop_rate(500ms);
  auto pose_publisher = publisher->create_publisher<std_msgs::msg::String>("topic", 10);
  auto get_data_client = node->create_client<getRobotData>("add_two_ints");
  ...
  // executor->add_node(subscription1);
  // executor->add_node(subscription2);
  executor->spin();
